...
// Suscriptores SSE (res -> { empresaId, esSuperadmin })
const sseSubs = new Map();
...
function addSseSubscriber(res, empresaId, esSuperadmin) {
  sseSubs.set(res, { empresaId: empresaId || null, esSuperadmin: !!esSuperadmin });
}
...
function broadcast(obj) {
  ...
  for (const [res, sub] of sseSubs) {
    if (!sub.esSuperadmin && (sub.empresaId == null || sub.empresaId !== obj.empresaId)) continue;
    try { res.write(payload); } catch (e) { sseSubs.delete(res); }
  }
}

// Persiste estado de la máquina y avisa a los navegadores por SSE.
async function actualizarMaquina(id, empresaId, patch) {
  for (let i = 0; i < 3; i++) {
    try {
  ...
  broadcast({ maquinaId: id, empresaId, ...patch });
}

// Maneja un mensaje entrante ya parseado de un dispositivo autenticado.
async function _onMessage(id, empresaId, ip, msg) {
  switch (msg.t) {
    case proto.HELLO:
    case proto.TELEMETRIA:
      await actualizarMaquina(id, empresaId, {
      ...
      try { await procesos.registrarMuestra(empresaId, id, msg.serial, msg.estado); } catch (e) { /* no romper el gateway */ }
      break;
    case proto.OTA_PROGRESO:
      broadcast({ maquinaId: id, empresaId, otaPct: msg.pct });
      break;
    case proto.PONG:
      break;
    default:
  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', async (raw) => {
      ...
      if (!autenticado) {
        ...
        _register(id, ws);
        await _onMessage(id, empresaId, ip, msg);
        return;
      }
      await _onMessage(id, empresaId, ip, msg);
    });
    ...
    ws.on('close', async () => {
      clearTimeout(authTimer);
      if (id) { _unregister(id, ws); await actualizarMaquina(id, empresaId, { online: false, ultimoVisto: new Date().toISOString() }); }
    });
